//const newsList = document.querySelector('.news:last-child .news-list');
...
export const loadNews = (country) => {
    document.querySelector('.news:last-child .news-list').remove();
    const ul = document.createElement('ul');
    ul.className = 'news-list';
    document.querySelector('.news:last-child .container').append(ul);
    ...
    fetch(`https://newsapi.org/v2/top-headlines?country=${country}`, {
        ...
        .then((result) => {
            result.articles.forEach((item) => {
                ...
                document.querySelector('.news:last-child .news-list').append(li);
            });
        });
};
